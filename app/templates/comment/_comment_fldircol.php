      ...
      <div fl jucsb alic gap=smol>
        <div fl alic gap=smol>
          <div fl alic jucc circled color=light
            style="height:36px;min-width:36px;max-width:1.8em;margin-top:1px;border:1px solid rgba(255,255,255,.32);z-index:1;background:<?= $Visitor->color ?>">
            <p text semibold smol ttup><?= substr($Comment->visitor->nickname, 0, 2) ?></p>
          </div>
          <div>
            <p text smolplus semibold word-wrap lh1 style="color:<?= $Visitor->color ?>">
              <?= $Comment->visitor->nickname ?></p>
